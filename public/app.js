...
const notationState = {voice:0,kind:'note',duration:1,accidental:'natural'};
...
function notationEventSvg(event,x,y,staffTop,staffBottom) {
  const duration=event.duration,up=y>108,stemX=up?x+7:x-7,stemEnd=up?y-42:y+42;
  if(event.rest){
    if(duration===4)return `<g class="engraved rest-mark" aria-label="whole rest"><rect x="${x-9}" y="${staffTop+36}" width="18" height="7" rx="1"/></g>`;
    if(duration===2)return `<g class="engraved rest-mark" aria-label="half rest"><rect x="${x-9}" y="${staffTop+47}" width="18" height="7" rx="1"/></g>`;
  ...
  const open=duration>=2,head=`<ellipse cx="${x}" cy="${y}" rx="9" ry="6.5" transform="rotate(-18 ${x} ${y})" class="note-head${open?' open':''}"/>`;
  const stem=duration===4?'':`<line class="note-stem" x1="${stemX}" y1="${y}" x2="${stemX}" y2="${stemEnd}"/>`;
  const flag=duration===.5?`<path class="note-flag" d="M${stemX} ${stemEnd}q${up?20:-20} 8 ${up?12:-12} 25"/>`:'';
  return `<g class="engraved note-mark" aria-label="${event.note}, ${durationLabel(duration)} note">${ledger.join('')}${accidental?`<text class="score-accidental" x="${x-22}" y="${y+7}">${accidental}</text>`:''}${head}${stem}${flag}</g>`;
}
...
function renderScoreEditor() {
  ...
  const voice=voices[notationState.voice],events=parseEvents(voice),sounded=events.filter(event=>!event.rest),averageMidi=sounded.length?sounded.reduce((sum,event)=>sum+event.midi,0)/sounded.length:64,bassClef=averageMidi<60,staffBase=bassClef?pitchIndex('G',2):pitchIndex('E',4),staffTop=72,staffBottom=144,beatWidth=58,total=Math.max(events.reduce((n,e)=>n+e.duration,0),16),width=Math.max(1200,150+total*beatWidth);
  svg.setAttribute('viewBox',`0 0 ${width} 250`);svg.style.width=`${Math.max(100,width/12)}%`;svg.setAttribute('tabindex','0');
  const staff=Array.from({length:5},(_,i)=>`<line x1="76" y1="${staffTop+i*18}" x2="${width-30}" y2="${staffTop+i*18}"/>`).join('');
  const bars=Array.from({length:Math.floor(total/4)+1},(_,i)=>{const x=130+i*4*beatWidth;return `<line class="barline" x1="${x}" y1="${staffTop}" x2="${x}" y2="${staffBottom}"/><text class="measure-number" x="${x+5}" y="${staffTop-14}">${i+1}</text>`;}).join('');
  let cursor=0;const marks=events.map(event=>{const x=155+(cursor+event.duration/2)*beatWidth;cursor+=event.duration;const bare=event.note.replace(/[#b]/,'');const match=bare.match(/^([A-G])(\-?\d)$/i),y=event.rest?108:staffBottom-(pitchIndex(match[1].toUpperCase(),Number(match[2]))-staffBase)*9;return `<g class="score-event"><title>Beat ${event.start+1}: ${event.rest?'rest':event.note}, ${durationLabel(event.duration)}</title>${notationEventSvg(event,x,y,staffTop,staffBottom)}</g>`;}).join('');
  svg.innerHTML=`<rect class="score-hit-area" width="${width}" height="250"/><g class="staff-lines">${staff}</g><text class="${bassClef?'bass-clef':'treble-clef'}" x="82" y="${bassClef?137:151}">${bassClef?'𝄢':'𝄞'}</text><text class="time-signature" x="118" y="101">4</text><text class="time-signature" x="118" y="137">4</text><g class="measure-lines">${bars}</g>${marks}<line class="end-bar" x1="${width-34}" y1="${staffTop}" x2="${width-34}" y2="${staffBottom}"/>`;
  svg.setAttribute('aria-label',`${voice.name}: ${events.length} notated events on a treble staff. Click a line or space to append a ${durationLabel(notationState.duration)} ${notationState.kind}.`);
}
...
document.querySelectorAll('[data-kind]').forEach(button=>button.addEventListener('click',()=>{notationState.kind=button.dataset.kind;selectNotationTool('[data-kind]','kind',notationState.kind);$('score-instruction').textContent=notationState.kind==='rest'?'Click the staff to place the selected rest value.':'Click any line or space to add a note. The score advances from left to right.';renderScoreEditor();}));
...
$('notation-staff')?.addEventListener('click',event=>{const svg=$('notation-staff'),point=svg.createSVGPoint();point.x=event.clientX;point.y=event.clientY;const local=point.matrixTransform(svg.getScreenCTM().inverse()),voices=parseVoices(),voice=voices[notationState.voice];if(!voice)return;let token;if(notationState.kind==='rest')token=`R/${notationState.duration}`;else{const sounded=parseEvents(voice).filter(item=>!item.rest),average=sounded.length?sounded.reduce((sum,item)=>sum+item.midi,0)/sounded.length:64,base=average<60?pitchIndex('G',2):pitchIndex('E',4),index=base+Math.round((144-Math.max(36,Math.min(198,local.y)))/9),natural=pitchFromIndex(index),mark=notationState.accidental==='sharp'?'#':notationState.accidental==='flat'?'b':'';token=`${natural[0]}${mark}${natural.slice(1)}/${notationState.duration}`;}voice.notes=`${voice.notes} ${token}`.trim();setScoreSource(voices);});
