...
$('refresh').addEventListener('click',loadHistory);$('export-analysis').addEventListener('click',exportReading);notes.addEventListener('input',syncVoices);notes.addEventListener('keydown',event=>{if((event.ctrlKey||event.metaKey)&&event.key==='Enter')form.requestSubmit();});voiceSelect.addEventListener('change',()=>{stopAudio();if(!results.classList.contains('hidden'))renderTexture();});tempo.addEventListener('input',()=>{$('tempo-value').textContent=`${tempo.value} BPM`;});$('play-texture').addEventListener('click',()=>playVoices(false));$('play-focus').addEventListener('click',()=>playVoices(true));$('stop-audio').addEventListener('click',stopAudio);document.querySelectorAll('[data-layer]').forEach(button=>button.addEventListener('click',()=>{const layer=button.dataset.layer;if(activeLayers.has(layer))activeLayers.delete(layer);else activeLayers.add(layer);button.classList.toggle('active',activeLayers.has(layer));button.setAttribute('aria-pressed',String(activeLayers.has(layer)));if(!results.classList.contains('hidden'))renderTexture();}));if(/^Archived voice\s*:/i.test(notes.value))notes.value=notes.value.replace(/^Archived voice\s*:/i,'Voice I:');syncVoices();loadHistory();
