...
function renderScoreEditor() {
  ...
  const [meterCount,meterValue]=meterParts();$('measure-count').textContent=notationState.measures;$('remove-measure').disabled=notationState.measures<=requiredMeasures;$('remove-voice').disabled=voices.length<=1;$('delete-event').disabled=!notationState.selectedEvent;$('voice-name').value=voices[notationState.voice]?.name||'';$('voice-clef').value=notationState.clefs[notationState.voice]||'auto';$('score-summary').innerHTML=`<strong>${notationState.measures} measure${notationState.measures===1?'':'s'} · ${notationState.timeSignature}</strong><span>${meterPulse()} · click the staff to append an event</span>`;
...
$('add-voice')?.addEventListener('click',()=>{const voices=parseVoices(),number=voices.length+1;voices.push({name:`Voice ${number}`,notes:'R/1'});notationState.clefs.push('auto');notationState.voice=voices.length-1;notationState.selectedEvent=null;setScoreSource(voices);voiceSelect.value=notationState.voice;});
