...
sampleButtons.forEach(button => button.addEventListener('click', () => {
  stopAudio(); notationState.title=button.querySelector('strong')?.textContent||'';notationState.selectedEvent=null;$('score-title').value=notationState.title;notes.value = button.dataset.notes; syncVoices(); if(!results.classList.contains('hidden')){results.classList.add('is-stale');$('saved-badge').textContent='Score changed · analyze again';}$('score-instruction').innerHTML=`<strong>${escapeText(notationState.title)} loaded.</strong> Select an event to inspect it, or choose a value and continue the score.`;$('notation-staff')?.focus();
...
function parseEvents(voice, strict = false) {
  ...
  return voice.notes.split(/[\s,]+/).filter(Boolean).map(token => {
    ...
    const durationPart = parts[1] || '1', tuplet = /t$/i.test(durationPart), notatedDuration = Number(durationPart.replace(/t$/i,''));
    if (!Number.isFinite(notatedDuration) || notatedDuration <= 0 || notatedDuration > 16) { if (strict) throw new Error(`${voice.name}: invalid duration in “${token}”.`); return null; }
    const duration = notatedDuration * (tuplet ? 2/3 : 1);
    ...
    const event = {note:parts[0], midi, start:cursor, duration, notatedDuration, tuplet, rest}; cursor += duration; return event;
  }).filter(Boolean);
}
...
const notationState = {voice:0,kind:'note',duration:1,triplet:false,accidental:'natural',timeSignature:'4/4',measures:4,title:'',clefs:[],selectedEvent:null};
...
const durationLabel = (duration,tuplet=false) => `${({'0.0625':'sixty-fourth','0.125':'thirty-second','0.25':'sixteenth','0.5':'eighth','1':'quarter','2':'half','4':'whole'}[String(duration)] || `${duration}-beat`)}${tuplet?' triplet':''}`;
const eventDurationLabel = event => durationLabel(event.notatedDuration ?? event.duration,event.tuplet);
...
function notationEventSvg(event,x,y,staffTop,staffBottom,options={}) {
  const duration=event.notatedDuration??event.duration,up=options.stemUp??y>=(staffTop+staffBottom)/2,stemX=up?x+7:x-7,stemEnd=options.stemEnd??(up?y-46:y+46);
  if(event.rest){
    const rests={'0.0625':['𝅁','sixty-fourth',2],'0.125':['𝅀','thirty-second',2],'0.25':['𝄿','sixteenth',2],'0.5':['𝄾','eighth',2],'1':['𝄽','quarter',2],'2':['𝄼','half',5],'4':['𝄻','whole',0]},[glyph,label,offset]=rests[String(duration)]||rests['1'];
  ...
  const flags=!options.beamed?Array.from({length:flagCount(duration)},(_,index)=>{const sy=stemEnd+(up?index*8:-index*8),direction=up?1:-1;return `<path class="note-flag" d="M${stemX} ${sy} C ${stemX+18} ${sy+3*direction}, ${stemX+20} ${sy+16*direction}, ${stemX+7} ${sy+25*direction} C ${stemX+13} ${sy+15*direction}, ${stemX+11} ${sy+9*direction}, ${stemX} ${sy+8*direction} Z"/>`;}).join(''):'';
  return `<g class="engraved note-mark" aria-label="${event.note}, ${eventDurationLabel(event)} note">${ledger.join('')}${accidental?`<text class="score-accidental" x="${x-22}" y="${y+7}">${accidental}</text>`:''}${stem}${flags}${head}</g>`;
}
...
function renderScoreEditor() {
  ...
  if(notationState.selectedEvent&&notationState.selectedEvent.voice>=voices.length)notationState.selectedEvent=null;
  ...
  const systems=voices.map((voice,voiceIndex)=>{
    ...
    for(let i=0;i<layouts.length;){
      ...
      const run=[i];while(run.length<8&&i+run.length<layouts.length){const previous=layouts[run.at(-1)].event,next=layouts[i+run.length].event;if(next.rest||next.tuplet!==previous.tuplet||next.notatedDuration!==previous.notatedDuration||Math.abs(previous.start+previous.duration-next.start)>.001||Math.floor(previous.start+.0001)!==Math.floor(next.start+.0001))break;run.push(i+run.length);}
      if(run.length<2){i++;continue;}
      const stemUp=run.reduce((sum,index)=>sum+layouts[index].y,0)/run.length>=(staffTop+staffBottom)/2,firstY=layouts[run[0]].y,lastY=layouts[run.at(-1)].y,slope=Math.max(-10,Math.min(10,lastY-firstY)),firstEnd=stemUp?firstY-46:firstY+46;
      run.forEach((index,position)=>{layouts[index].options={beamed:true,stemUp,stemEnd:firstEnd+slope*position/(run.length-1)};});
      const a=layouts[run[0]],b=layouts[run.at(-1)],ax=a.x+(stemUp?7:-7),bx=b.x+(stemUp?7:-7),direction=stemUp?1:-1;
      for(let level=0;level<count;level++){const ay=a.options.stemEnd+level*8*direction,by=b.options.stemEnd+level*8*direction,thickness=5*direction;beams.push(`<path class="note-beam" d="M${ax} ${ay} L${bx} ${by} L${bx} ${by+thickness} L${ax} ${ay+thickness} Z"/>`);}
      i=run.at(-1)+1;
    }
  ...
  svg.setAttribute('aria-label',`${voices.length}-voice score with ${voices.map(voice=>voice.name).join(', ')}. Click any staff to append a ${selectedDurationLabel()} ${notationState.kind}.`);
}
...
function updateToolStatus() {
  const rest=notationState.kind==='rest',label=`${selectedDurationLabel()} ${rest?'rest':'note'}`;
  document.querySelectorAll('[data-accidental]').forEach(button=>button.disabled=rest);
  $('score-instruction').innerHTML=`<strong>${label[0].toUpperCase()+label.slice(1)} selected.</strong> ${rest?'Click any voice staff to add it.':'Click a line or space on any staff to add it.'} The score advances from left to right.`;
}

function adoptSelectedEvent(event) {
  notationState.kind=event.rest?'rest':'note';notationState.duration=event.notatedDuration;notationState.triplet=event.tuplet;
  if(!event.rest)notationState.accidental=event.note.includes('#')?'sharp':event.note.includes('b')?'flat':'natural';
  selectNotationTool('[data-kind]','kind',notationState.kind);selectNotationTool('[data-duration]','duration',notationState.duration);selectNotationTool('[data-accidental]','accidental',notationState.accidental);
  const triplet=$('triplet-tool');triplet.classList.toggle('active',notationState.triplet);triplet.setAttribute('aria-pressed',String(notationState.triplet));updateToolStatus();
}

document.querySelectorAll('[data-kind]').forEach(button=>button.addEventListener('click',()=>{notationState.kind=button.dataset.kind;selectNotationTool('[data-kind]','kind',notationState.kind);updateToolStatus();renderScoreEditor();}));
document.querySelectorAll('[data-duration]').forEach(button=>button.addEventListener('click',()=>{notationState.duration=Number(button.dataset.duration);selectNotationTool('[data-duration]','duration',button.dataset.duration);updateToolStatus();renderScoreEditor();}));
$('triplet-tool')?.addEventListener('click',event=>{notationState.triplet=!notationState.triplet;event.currentTarget.classList.toggle('active',notationState.triplet);event.currentTarget.setAttribute('aria-pressed',String(notationState.triplet));updateToolStatus();renderScoreEditor();});
...
$('score-title')?.addEventListener('input',event=>{notationState.title=event.target.value.trim();});
$('voice-name')?.addEventListener('change',event=>{const voices=parseVoices(),voice=voices[notationState.voice],name=event.target.value.trim();if(!voice||!name){renderScoreEditor();return;}voice.name=name;setScoreSource(voices);});
$('voice-clef')?.addEventListener('change',event=>{notationState.clefs[notationState.voice]=event.target.value;renderScoreEditor();});
$('score-voices')?.addEventListener('click',event=>{const button=event.target.closest('[data-score-voice]');if(!button)return;notationState.voice=Number(button.dataset.scoreVoice);notationState.selectedEvent=null;voiceSelect.value=notationState.voice;renderScoreEditor();});
$('add-voice')?.addEventListener('click',()=>{const voices=parseVoices(),number=voices.length+1;voices.push({name:`Voice ${number}`,notes:''});notationState.clefs.push('auto');notationState.voice=voices.length-1;notationState.selectedEvent=null;setScoreSource(voices);voiceSelect.value=notationState.voice;updateToolStatus();});
$('remove-voice')?.addEventListener('click',()=>{const voices=parseVoices();if(voices.length<=1)return;voices.splice(notationState.voice,1);notationState.clefs.splice(notationState.voice,1);notationState.voice=Math.max(0,Math.min(notationState.voice,voices.length-1));notationState.selectedEvent=null;setScoreSource(voices);voiceSelect.value=notationState.voice;});
$('notation-staff')?.addEventListener('click',event=>{const selectedMark=event.target.closest?.('.score-event');if(selectedMark){notationState.voice=Number(selectedMark.dataset.eventVoice);notationState.selectedEvent={voice:notationState.voice,event:Number(selectedMark.dataset.eventIndex)};voiceSelect.value=notationState.voice;const voice=parseVoices()[notationState.voice],selected=parseEvents(voice)[notationState.selectedEvent.event];adoptSelectedEvent(selected);renderScoreEditor();$('score-instruction').innerHTML=`<strong>${escapeText(voice.name)} · beat ${Number((selected.start+1).toFixed(3))}: ${selected.rest?'rest':escapeText(selected.note)}, ${eventDurationLabel(selected)}.</strong> Delete it, or click empty staff space to continue with this value.`;return;}const svg=$('notation-staff'),point=svg.createSVGPoint();point.x=event.clientX;point.y=event.clientY;const local=point.matrixTransform(svg.getScreenCTM().inverse()),voices=parseVoices(),staffGap=166,voiceIndex=Math.max(0,Math.min(voices.length-1,Math.round((local.y-78)/staffGap))),voice=voices[voiceIndex];if(!voice)return;notationState.voice=voiceIndex;notationState.selectedEvent=null;voiceSelect.value=voiceIndex;let token;if(notationState.kind==='rest')token=`R/${durationToken()}`;else{const events=parseEvents(voice),sounded=events.filter(item=>!item.rest),average=sounded.length?sounded.reduce((sum,item)=>sum+item.midi,0)/sounded.length:64,clefChoice=notationState.clefs[voiceIndex]||'auto',bassClef=clefChoice==='bass'||(clefChoice==='auto'&&average<60),base=bassClef?pitchIndex('G',2):pitchIndex('E',4),staffTop=42+voiceIndex*staffGap,staffBottom=staffTop+72,index=base+Math.round((staffBottom-Math.max(staffTop-36,Math.min(staffBottom+54,local.y)))/9),natural=pitchFromIndex(index),mark=notationState.accidental==='sharp'?'#':notationState.accidental==='flat'?'b':'';token=`${natural[0]}${mark}${natural.slice(1)}/${durationToken()}`;}voice.notes=`${voice.notes} ${token}`.trim();setScoreSource(voices);updateToolStatus();});
$('delete-event')?.addEventListener('click',()=>{if(!notationState.selectedEvent)return;const voices=parseVoices(),voice=voices[notationState.selectedEvent.voice];if(!voice)return;const tokens=voice.notes.split(/[\s,]+/).filter(Boolean);tokens.splice(notationState.selectedEvent.event,1);voice.notes=tokens.join(' ');notationState.selectedEvent=null;setScoreSource(voices);updateToolStatus();});
$('undo-note')?.addEventListener('click',()=>{const voices=parseVoices(),voice=voices[notationState.voice];if(!voice)return;const tokens=voice.notes.split(/[\s,]+/).filter(Boolean);tokens.pop();voice.notes=tokens.join(' ');notationState.selectedEvent=null;setScoreSource(voices);updateToolStatus();});
$('clear-voice')?.addEventListener('click',()=>{const voices=parseVoices(),voice=voices[notationState.voice];if(!voice)return;voice.notes='';notationState.selectedEvent=null;setScoreSource(voices);$('score-instruction').innerHTML=`<strong>${escapeText(voice.name)} is empty.</strong> Choose a note or rest value, then click its staff to begin again.`;});
$('notation-staff')?.addEventListener('keydown',event=>{if(event.ctrlKey||event.metaKey)return;if(event.key==='Backspace'||event.key==='Delete'){event.preventDefault();$(notationState.selectedEvent?'delete-event':'undo-note').click();return;}if(event.key.toLowerCase()==='m'){event.preventDefault();$('add-measure').click();return;}if(event.key.toLowerCase()==='t'){event.preventDefault();$('triplet-tool').click();return;}const durations={1:.5,2:1,3:2,4:4,5:.25,6:.125,7:.0625};if(durations[event.key]){event.preventDefault();notationState.duration=durations[event.key];selectNotationTool('[data-duration]','duration',String(durations[event.key]));updateToolStatus();renderScoreEditor();}});
...
async function playVoices(focusOnly=false){
  ...
  try {
    ...
    voices.forEach((voice,voiceIndex)=>{ if(focusOnly&&voiceIndex!==selected)return; parseEvents(voice,true).forEach(event=>{ if(event.rest)return; const oscillator=audioContext.createOscillator(),gain=audioContext.createGain(); oscillator.type=['triangle','sine','triangle'][voiceIndex%3]; oscillator.detune.value=(voiceIndex-(voices.length-1)/2)*2.5; oscillator.frequency.value=440*Math.pow(2,(event.midi-69)/12); const onset=start+event.start*beat,release=onset+Math.max(.012,event.duration*beat*.92),span=release-onset,attack=Math.min(.025,span*.3),hold=Math.max(onset+attack,release-Math.min(.08,span*.25)); latestRelease=Math.max(latestRelease,release); const level=focusOnly?.13:(voiceIndex===selected?textureLevel*2.25:textureLevel*.72); gain.gain.setValueAtTime(.0001,onset);gain.gain.exponentialRampToValueAtTime(level,onset+attack);gain.gain.setValueAtTime(level,hold);gain.gain.exponentialRampToValueAtTime(.0001,release);oscillator.connect(gain);if(audioContext.createStereoPanner&&voices.length>1){const panner=audioContext.createStereoPanner();panner.pan.value=-.58+voiceIndex*1.16/(voices.length-1);gain.connect(panner).connect(master);}else gain.connect(master);oscillator.start(onset);oscillator.stop(release+.02);activeSources.push(oscillator);});});
