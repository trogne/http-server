...
function renderTexture() {
  ...
  const lines = sounded.map((events, voiceIndex) => {
    ...
    return `<g class="texture-voice" style="--voice:${color};--delay:${delay}s"><path class="texture-line" pathLength="1" d="${path}"/>`+points.map((p,index)=>`<circle class="texture-point" style="--point-delay:${delay+index*.018}s" cx="${p.x}" cy="${p.y}" r="3.5"><title>${escapeText(voices[voiceIndex].name)} · ${escapeText(events[index].note)} · beat ${events[index].start+1}</title></circle>`).join('')+'</g>';
  }).join('');
