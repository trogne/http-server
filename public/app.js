...
function renderScoreEditor() {
  ...
  const timelines=voices.map(parseEvents),beatWidth=58,total=Math.max(16,...timelines.map(events=>events.reduce((sum,event)=>sum+event.duration,0))),width=Math.max(1200,210+total*beatWidth),staffGap=166,height=voices.length*staffGap+44;
  ...
  const systems=voices.map((voice,voiceIndex)=>{
    ...
    const staff=Array.from({length:5},(_,i)=>`<line x1="108" y1="${staffTop+i*18}" x2="${width-30}" y2="${staffTop+i*18}"/>`).join('');
    const bars=Array.from({length:Math.floor(total/4)+1},(_,i)=>{const x=174+i*4*beatWidth;return `<line class="barline" x1="${x}" y1="${staffTop}" x2="${x}" y2="${staffBottom}"/>${voiceIndex===0?`<text class="measure-number" x="${x+5}" y="${staffTop-13}">${i+1}</text>`:''}`;}).join('');
    let cursor=0;const marks=events.map(event=>{const x=199+(cursor+event.duration/2)*beatWidth;cursor+=event.duration;const bare=event.note.replace(/[#b]/,''),match=bare.match(/^([A-G])(\-?\d)$/i),y=event.rest?(staffTop+staffBottom)/2:staffBottom-(pitchIndex(match[1].toUpperCase(),Number(match[2]))-staffBase)*9;return `<g class="score-event"><title>${escapeText(voice.name)}, beat ${event.start+1}: ${event.rest?'rest':event.note}, ${durationLabel(event.duration)}</title>${notationEventSvg(event,x,y,staffTop,staffBottom)}</g>`;}).join('');
    return `<g class="score-system${selected?' selected-system':''}" data-system="${voiceIndex}"><rect class="staff-selector" x="0" y="${staffTop-30}" width="${width}" height="${staffGap-5}"/><text class="staff-name" x="91" y="${staffTop+41}" text-anchor="end">${escapeText(voice.name)}</text><g class="staff-lines">${staff}</g><text class="${bassClef?'bass-clef':'treble-clef'}" x="114" y="${bassClef?staffTop+65:staffTop+79}">${bassClef?'𝄢':'𝄞'}</text><text class="time-signature" x="160" y="${staffTop+29}">4</text><text class="time-signature" x="160" y="${staffTop+65}">4</text><g class="measure-lines">${bars}</g>${marks}<line class="end-bar" x1="${width-34}" y1="${staffTop}" x2="${width-34}" y2="${staffBottom}"/></g>`;
  }).join('');
  const firstTop=42,lastBottom=42+(voices.length-1)*staffGap+72;
  svg.innerHTML=`<rect class="score-hit-area" width="${width}" height="${height}"/><path class="system-brace" d="M101 ${firstTop}q-14 0-14 16v${Math.max(0,lastBottom-firstTop-32)}q0 16 14 16"/>${systems}`;
