...
function renderScoreEditor() {
  ...
  const timelines=voices.map(parseEvents),beatWidth=64,total=Math.max(16,...timelines.map(events=>events.reduce((sum,event)=>sum+event.duration,0))),displayBeats=Math.ceil(total/4)*4,barStart=196,endBarX=barStart+displayBeats*beatWidth,width=Math.max(1200,endBarX+38),staffGap=166,height=voices.length*staffGap+44;
