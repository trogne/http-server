...
const notationState = {voice:0,kind:'note',duration:1,accidental:'natural',timeSignature:'4/4',measures:4,title:'',clefs:[],selectedEvent:null};
...
$('notation-staff')?.addEventListener('keydown',event=>{if(event.ctrlKey||event.metaKey)return;if(event.key==='Backspace'||event.key==='Delete'){event.preventDefault();$(notationState.selectedEvent?'delete-event':'undo-note').click();return;}if(event.key.toLowerCase()==='m'){event.preventDefault();$('add-measure').click();return;}const durations={1:.5,2:1,3:2,4:4};if(durations[event.key]){event.preventDefault();notationState.duration=durations[event.key];selectNotationTool('[data-duration]','duration',String(durations[event.key]));renderScoreEditor();}});
